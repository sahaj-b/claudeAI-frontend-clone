      ...
      <div className={`p-3 border-l border-r border-b border-borderclr2 rounded-b-2xl relative max-w-2xl m-auto bg-bg3 flex flex-col transition-all ease-in-out duration-300 ${hideSuggestions ? 'max-h-14 ' : 'max-h-80'}`}>
        ...
        <div className={`flex`}>
          ...
          <div className='flex flex-col m-auto space-y-1'>
            <Button value={<i className={`nf nf-cod-close opacity-70 hover:opacity-100 text-xs ${hideSuggestions ? 'hidden' : ''}`}></i>} tooltip="Hide suggestions" tooltip_direction="right" onClick_function={() => { setHideSuggestions(true) }} />
            <Button value={<i className={`nf nf-fa-refresh opacity-70 hover:opacity-100 text-xs ${hideSuggestions ? 'hidden' : ''}`}></i>} tooltip="Reload suggestions" tooltip_direction="right" />
          </div>
        </div>
      </div>
    </div>
